...
export class AccountAttackLimiter {
  constructor({ scheduler = null } = {}) {
    ...
    this._gcTimer = scheduler?.every
      ? scheduler.every('account-attack-limiter-gc', 5 * 60 * 1000, () => this._gc())
      : setInterval(() => this._gc(), 5 * 60 * 1000);
  ...
  stop() {
    if (typeof this._gcTimer?.cancel === 'function') this._gcTimer.cancel();
    else if (this._gcTimer) clearInterval(this._gcTimer);
    this._gcTimer = null;
  }
...
export const attackLimiter = new AccountAttackLimiter({ scheduler: runtimeGovernor.scheduler });
