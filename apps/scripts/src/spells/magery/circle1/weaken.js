...
export default {
  ...
  cast(api, ctx, target) {
    ...
    if (!target || !target.serial) { ctx.state.sendSystemMessage('Weaken needs a target.'); return; }
    ...
    broadcastSound(api, api.world, target, 0x1E6);
    if (api.statusEffects) applySingleStatBuff(api, target, 'weaken', 'str', -10, 60_000);
  },
};
