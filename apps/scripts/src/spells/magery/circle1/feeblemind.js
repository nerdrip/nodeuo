...
export default {
  ...
  cast(api, ctx, target) {
    ...
    if (!target || !target.serial) { ctx.state.sendSystemMessage('Feeblemind needs a target.'); return; }
    ...
    broadcastSound(api, api.world, target, 0x1E4);
    if (api.statusEffects) applySingleStatBuff(api, target, 'feeblemind', 'int', -10, 60_000);
  },
};
