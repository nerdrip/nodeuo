...
export default {
  ...
  cast(api, ctx, target) {
    ...
    if (!target || !target.serial) { ctx.state.sendSystemMessage('Cunning needs a target.'); return; }
    ...
    broadcastSound(api, api.world, target, 0x1EB);
    if (api.statusEffects) applySingleStatBuff(api, target, 'cunning', 'int', +10, 60_000);
  },
};
