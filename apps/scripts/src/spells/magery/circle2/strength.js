...
export default {
  ...
  cast(api, ctx, target) {
    ...
    if (!target || !target.serial) { ctx.state.sendSystemMessage('Strength needs a target.'); return; }
    ...
    broadcastSound(api, api.world, target, 0x1EE);
    if (api.statusEffects) applySingleStatBuff(api, target, 'strength', 'str', +10, 60_000);
  },
};
