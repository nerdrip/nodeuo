...
const TRANSPORT_GATE_RX = /(moongate|resgate|instanceexitgate)/i;

function looksLikeDoor(type) {
  return typeof type === 'string' && DOOR_RX.test(type) && !TRANSPORT_GATE_RX.test(type);
}
...
function loadTransportGateEntries() {
  if (!existsSync(DECO_PATH)) return [];
  try {
    const all = JSON.parse(readFileSync(DECO_PATH, 'utf8'));
    return all.filter((e) => typeof e.type === 'string' && TRANSPORT_GATE_RX.test(e.type));
  } catch { return []; }
}
...
function purgeLegacyTransportDoors(api) {
  const world = api.world;
  const transportEntries = loadTransportGateEntries();
  const victims = [];
  for (const it of allItems({ world })) {
    if (it.script !== 'door' || !it.isDecoration) continue;
    const hit = transportEntries.some((e) => e.map === it.map
      && Math.abs((e.x | 0) - (it.x | 0)) <= 1
      && Math.abs((e.y | 0) - (it.y | 0)) <= 1
      && Math.abs((e.z | 0) - (it.z | 0)) <= 1
      && (it.itemId === e.itemId || it.itemId === e.itemId + 1));
    if (hit) victims.push(it.serial);
  }
  for (const serial of victims) destroyItemBySerial(api, serial);
  if (victims.length) {
    api.log?.(`[doorgen] removed ${victims.length} transport gate(s) misclassified as doors`);
  }
...
export function applyDoors(api, opts = {}) {
  ...
  // Migration for worlds generated by the old broad /gate/ classifier:
  // public/resurrection/instance gates were persisted as solid doors. Remove
  // those legacy objects; their proper moongate/teleporter stage owns them.
  purgeLegacyTransportDoors(api);
...
export default function register(api) {
  ...
  // Run the migration at script load as well as during [doorgen], so a shard
  // with an existing persisted createworld save repairs blocked portals on
  // its next normal restart without requiring an admin command.
  if (purgeLegacyTransportDoors(api) > 0) {
    refreshOnlineClients(api);
    queueSave(api);
  }
